...
import { Search, Filter, Star, MapPin, TrendingUp, Calendar, Sparkles, Briefcase, Loader2, Zap } from 'lucide-react';
...
export default function Marketplace({ onNavigate, isLoggedIn, userRole }: MarketplaceProps) {
  ...
  const [activeTab, setActiveTab] = useState<'quick-buys' | 'projects'>('quick-buys');
  ...
  const filteredProjects = projects.filter((project) => {
    ...
    // Filter by tab (quick-buys vs projects)
    const isQuickBuy = project.type === 'direct';
    const matchesTab = activeTab === 'quick-buys' ? isQuickBuy : !isQuickBuy;
        ...
        {!loading && sortedProjects.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {sortedProjects.filter(p => p && p.id).map((project) => {
              ...
              // Use QuickBuyCard for direct purchases
              ...
              // Standard card for negotiable projects
              return (
                <Card
                  key={project.id}
